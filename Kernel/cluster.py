...
objects=np.zeros((134,2))
clusterData=np.copy(getData(objects,'similarity_k.csv'))
...
distanceArray = splitDistanceArray(generateDistanceArray(clusterData))
printcluster = clusterArray(clusterData,distanceArray,29)
with open('cluster_k.csv', 'wb') as f:
    writer = csv.writer(f)
    writer.writerows(printcluster)